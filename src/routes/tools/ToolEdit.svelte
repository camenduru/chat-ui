	...
	<div class="grid h-full w-full flex-1 grid-cols-2 gap-6 text-sm max-sm:grid-cols-1">
		<div class="col-span-1 flex flex-col gap-4">
			<div class="flex flex-col gap-4">
				...
				<p class="text-justify text-neutral-800">
		...
		<div class="col-span-1 flex flex-col gap-4">
			<div class="flex flex-col gap-2">
				...
				{#if editableTool.baseUrl}
					<p class="text-sm text-neutral-500">Choose functions that can be called in your tool.</p>
				{:else}
					<p class="text-sm text-neutral-500">Start by specifying a Hugging Face Space URL.</p>
				{/if}
					...
					{:then api}
						<div class="flex flex-row flex-wrap gap-4">
							{#each Object.keys(api["named_endpoints"] ?? {}) as name}
								<label class="rounded-lg bg-neutral-200 p-2">
									<input
										type="radio"
										disabled={readonly}
									...
									/>
									<span
										class="font-mono text-neutral-800"
						...
						{#if editableTool.endpoint && api["named_endpoints"][editableTool.endpoint] && !APIloading}
							...
							<div class="flex flex-col gap-2">
								<div class="flex flex-col gap-2 rounded-lg border border-neutral-200 p-2">
									<div class="flex items-center gap-1 border-b border-neutral-200 p-1 pb-2">
										...
										<label class="ml-auto">
											<span
												class="group relative flex w-max items-center justify-center text-sm font-semibold text-neutral-700"
											>
												AI Function Name
												<CarbonInformation class="m-1 align-middle text-xs text-purple-500" />
												<div
													class="pointer-events-none absolute -top-16 right-0 w-max rounded-md bg-neutral-100 p-2 opacity-0 transition-opacity group-hover:opacity-100 dark:bg-neutral-800"
												>
													<p class="max-w-sm text-sm font-normal text-neutral-800 dark:text-neutral-200">
											...
											<input
												class="h-fit rounded-lg border-2 border-neutral-200 bg-neutral-100 p-1"
												type="text"
												name="name"
												disabled={readonly}
									...
									{#each editableTool.inputs as input, inputIdx}
											...
											<div class="inline w-full">
												...
												<div class="flex w-fit justify-start gap-2">
													<label class="ml-auto">
														...
														/>
														<span class="text-sm text-neutral-500">Required</span>
													</label>
													<label class="ml-auto">
														<input
															type="radio"
													...
													<label class="ml-auto">
														<input
															...
															bind:group={editableTool.inputs[inputIdx].paramType}
														/>
														<span class="text-sm text-neutral-500">Fixed</span>
													</label>
												</div>
											</div>
										</div>
										...
										{#if input.type === "file"}
											<label class="flex flex-row gap-2">
												...
												<select
													...
													class="h-fit w-full rounded-lg border-2 border-neutral-200 bg-neutral-100 p-2"
										...
										<!-- divider -->
										<div
											class="flex w-full flex-row flex-nowrap gap-2 border-b border-neutral-200 pt-2"
										/>
									{/each}
									...
									<div class="flex flex-col gap-4">
													...
													<div class="flex flex-row gap-4">
														{#each api.named_endpoints[editableTool.endpoint].returns as { component }, idx}
															<label class="text-neutral-800">
																...
																<span
																	class="font-mono"
																	class:text-neutral-400={!ToolOutputComponents.safeParse(component)
																		.success}
										...
										<label class="flex flex-row gap-2" for="showOutput">
											...
											<input
												...
												class="peer rounded-lg border-2 border-neutral-200 bg-neutral-100 p-1"
											/>
											<p class="text-xs text-red-500">
						...
						{:else if APIloading}
							<p class="text-sm text-neutral-500">Loading API...</p>
			...
			<div class="relative bottom-0 mb-4 mt-auto flex w-full flex-row justify-end gap-2">
				<button
					type="button"
					class="mt-4 w-fit rounded-full bg-neutral-200 px-4 py-2 font-semibold text-neutral-700"
				...
				{#if !readonly}
					<button
						...
						class:text-neutral-300={formLoading || !formSubmittable}
						class:bg-neutral-400={formLoading || !formSubmittable}
					>
