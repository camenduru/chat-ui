...
<Modal on:close>
	...
	>
		...
		<p class="text-lg font-semibold leading-snug text-neutral-800" style="text-wrap: balance;">
		...
		<div class="flex w-full flex-col items-center gap-2">
			<button
				class="w-full justify-center rounded-full border-2 border-neutral-300 bg-black px-5 py-2 text-lg font-semibold text-neutral-100 transition-colors hover:bg-neutral-800"
				class:bg-white={$page.data.loginEnabled}
				class:text-neutral-800={$page.data.loginEnabled}
			...
			{#if $page.data.loginEnabled}
				<form action="{base}/login" target="_parent" method="POST" class="w-full">
					<button
						type="submit"
						class="flex w-full flex-wrap items-center justify-center whitespace-nowrap rounded-full border-2 border-black bg-black px-5 py-2 text-lg font-semibold text-neutral-100 transition-colors hover:bg-neutral-800"
					>
						Sign in
