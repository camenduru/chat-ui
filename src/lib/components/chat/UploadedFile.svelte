	...
	<Modal width="sm:max-w-[800px]" on:close={() => (showModal = false)}>
		...
		{:else if isPlainText(file.mime)}
			<div class="relative flex h-full w-full flex-col gap-4 p-4">
				...
				{/if}
				<button
					class="absolute right-4 top-4 text-xl text-neutral-500 hover:text-neutral-800"
...
<button on:click={() => (showModal = true)} disabled={!isClickable} class:clickable={isClickable}>
	<div class="group relative flex items-center rounded-xl shadow-sm">
		{#if isImage(file.mime)}
			<div class="size-48 overflow-hidden rounded-xl">
				<img
					...
					alt={file.name}
					class="h-full w-full bg-neutral-200 object-cover dark:bg-neutral-800"
				/>
			</div>
		...
		{:else if isVideo(file.mime)}
			<div
				class="border-1 w-72 overflow-clip rounded-xl border-neutral-200 bg-white dark:border-neutral-800 dark:bg-neutral-800"
			>
		...
		{:else if isPlainText(file.mime)}
			<div
				class="flex h-14 w-72 items-center gap-2 overflow-hidden rounded-xl border border-neutral-200 bg-white p-2 dark:border-neutral-800 dark:bg-neutral-800"
				class:hoverable={isClickable}
			>
				<div
					class="grid size-10 flex-none place-items-center rounded-lg bg-neutral-100 dark:bg-neutral-800"
				>
					<CarbonDocument class="text-base text-neutral-700 dark:text-neutral-300" />
				</div>
				<dl class="flex flex-col items-start truncate leading-tight">
					...
					{#if file.mime === "application/vnd.chatui.clipboard"}
						<dt class="text-xs text-neutral-400">Clipboard source</dt>
					{:else}
						<dt class="text-xs text-neutral-400">{file.mime}</dt>
					{/if}
				</dl>
			</div>
		{:else if file.mime === "octet-stream"}
			...
			>
				<div
					class="grid size-10 flex-none place-items-center rounded-lg bg-neutral-100 dark:bg-neutral-800"
				>
					<CarbonDocumentBlank class="text-base text-neutral-700 dark:text-neutral-300" />
				</div>
				<dl class="flex flex-grow flex-col truncate leading-tight">
					<dd class="text-sm">
						{truncateMiddle(file.name, 28)}
					</dd>
					<dt class="text-xs text-neutral-400">File type could not be determined</dt>
				</dl>
				<a
				...
				>
					<CarbonDownload class="text-base text-neutral-700 dark:text-neutral-300" />
				</a>
			</div>
		{:else}
			<div
		...
		{#if canClose}
			<button
				class="absolute -right-2 -top-2 z-10 grid size-6 place-items-center rounded-full border bg-black group-hover:visible dark:border-neutral-700"
